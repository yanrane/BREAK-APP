/**
 * Avatar profil yang terbuka mengikuti level pet. Disimpan di `User.avatarUrl`
 * sebagai data-URI SVG emoji supaya semua <img src={avatarUrl}> yang sudah ada
 * (leaderboard, profil publik) langsung jalan tanpa perlu hosting file.
 */
...
export interface Avatar {
  ...
  emoji: string;
  label: string;
  /** Level pet minimum untuk memakai avatar ini. */
  level: number;
}
...
/** Emoji selalu dari AVATARS (bukan input user), jadi aman diinterpolasi ke SVG. */
...
/** Avatar yang boleh dipakai pada level pet tertentu. */
export function unlockedAvatars(level: number): Avatar[] {
  return AVATARS.filter((a) => a.level <= level);
}
