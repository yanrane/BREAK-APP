...
export default function Dashboard() {
    ...
    return (
      <div className="max-w-2xl space-y-4 animate-pulse">
        ...
        <div className="grid grid-cols-2 gap-3">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="h-28 bg-cream-2 border-2 border-ink/20" />
          ))}
        </div>
      </div>
    );
  ...
  if (error || !data) {
    return (
      <div className="max-w-2xl text-center py-16 border-2 border-ink shadow-hard">
        ...
        <button
          onClick={refetch}
          className="text-sm font-extrabold underline decoration-lime decoration-2"
        >
          Coba lagi
        </button>
      </div>
  ...
  const missionPct =
    data.todayMissions.total > 0
      ? Math.round(
          (data.todayMissions.completed / data.todayMissions.total) * 100,
        )
      : 0;
  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      {data.topLeaderboard.length > 0 && (
        <div className="border-2 border-ink shadow-hard">
          <div className="flex items-center justify-between border-b-2 border-ink px-5 py-3">
            ...
            <Link
              to="/leaderboard"
              className="text-xs font-extrabold underline decoration-lime decoration-2"
            >
              Lihat semua →
            </Link>
          </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <Link
          to="/missions"
          className="border-2 border-ink p-5 shadow-hard-aqua hover:shadow-hard-sm hover:translate-x-[2px] hover:translate-y-[2px] active:shadow-none active:translate-x-[4px] active:translate-y-[4px] transition-all duration-100 bg-cream group"
        >
        ...
        <Link
          to="/games"
          className="border-2 border-ink p-5 shadow-hard-grape hover:shadow-hard-sm hover:translate-x-[2px] hover:translate-y-[2px] active:shadow-none active:translate-x-[4px] active:translate-y-[4px] transition-all duration-100 bg-cream-2"
        >
          <p className="text-3xl mb-3">⚡</p>
